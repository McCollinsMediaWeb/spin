...
var wheelConfig;
var reelNumber = 0;
var dynamicUrl = "https://cleaningsuperstore.com/";
...
window.onload = function () {
  ...
  var gameConfig = {
    ...
    backgroundColor: 0xfdcd1a, // game background color
    scene: [FortuneWheelPink6],
    audio: {
...
class CommonWheel extends Phaser.Scene {
  ...
  preload() {
    ...
    this.useSpinArrow =
      this.getSpriteData("spinarrow") != null &&
      this.getSpriteData("spinarrow").fileName != null;
    this.usePointer =
    ...
    this.load.audio("pointer_hit_clip", [
      "https://github.com/McCollinsMediaWeb/spin/blob/main/audio/pointer_hit.ogg",
      "https://github.com/McCollinsMediaWeb/spin/blob/main/audio/pointer_hit.mp3",
    ]); // this.load.audio('wheel_spin_clip', 'audio/spin_sound.mp3'); this.load.audio('coins_clip', 'audio/win_coins.wav');
    this.load.audio("win_clip", ["https://github.com/McCollinsMediaWeb/spin/blob/main/audio/win_sound.ogg", "https://github.com/McCollinsMediaWeb/spin/blob/main/audio/win_sound.mp3"]);
  ...
  create() {
    ...
    this.centerY =
      fortuneWheelGame.config.height / 2 + wheelConfig.centerOffsetY;
    ...
    // adding the text field for spin button
    this.prizeText = this.add
    ...
    const cornerRadius = 20; // Border-radius for the background

    // Create a rounded rectangle for the button background
    this.buttonBackground = this.add.graphics();
    this.buttonBackground.fillStyle(0x3498db, 1); // Set background color (blue in this case)
    ...
    // Add the 'Go to Page' text on top of the button background
    this.redirectButton = this.add
      .bitmapText(
      ...
      .setOrigin(0.5)
      .setTint(0xffffff); // Set the text color to white

    // Make the background interactive, including the text
    this.buttonBackground.setInteractive(
      new Phaser.Geom.Rectangle(buttonX, buttonY, buttonWidth, buttonHeight),
      Phaser.Geom.Rectangle.Contains
    );
    this.buttonBackground.on("pointerdown", () => {
      window.location.href = dynamicUrl; // Redirect when the button background is clicked
    });

    // Make the text itself interactive, showing the hand cursor on hover
    this.redirectButton.setInteractive({ useHandCursor: true });
    this.redirectButton.setVisible(false);
    this.buttonBackground.setVisible(false);
    this.redirectButton.on("pointerdown", () => {
      window.location.href = dynamicUrl; // Redirect when the text is clicked
    });
    ...
    for (var i = 0; i < this.sectorsCount; i++) {
      ...
      this.sectorIcon = this.add
        .sprite(posX, posY, wheelConfig.sectors[i].icon)
        .setOrigin(0.5, 0.5);
    ...
    if (this.useSpinArrow)
      this.spinArrow = this.addSprite("spinarrow").setAlpha(0);
  ...
  spinWheel() {
    ...
    if (this.canSpin) {
      ...
      var rounds = Phaser.Math.Between(
        wheelConfig.wheelRounds.min,
        wheelConfig.wheelRounds.max
      );
  ...
  animLightSector() {
    ...
    this.lightTween = this.tweens.add({
      ...
      alphaTopLeft: {
        value: 1,
        duration: wheelConfig.lightTweenDuration,
        ease: "Power1",
      },
      alphaTopRight: {
        value: 1,
  ...
  addSprite(name) {
    ...
    return this.add
      .sprite(
        this.centerX + spriteData.offsetX,
        this.centerY + spriteData.offsetY,
        name
      )
      .setOrigin(spriteData.originX, spriteData.originY);
  }
  ...
  getSpriteData(spriteName) {
    for (var si = 0; si < wheelConfig.sprites.length; si++) {
      if (wheelConfig.sprites[si].name === spriteName)
        return wheelConfig.sprites[si];
    }
    return null;
  }
    ...
    if (this.lampsArray) {
      if (blink && !this.lampsIntervalID) {
        ...
        this.lampsIntervalID = setInterval(() => {
          this.lampsArray.forEach((l) => {
            l.setOn(this._lampsOn);
          });
  ...
  showNextReel() {
    if (this.lampsIntervalID) clearInterval(this.lampsIntervalID);
    console.log("reelNumber: " + reelNumber);
    if (reelNumber == 0) {
      this.scene.start("FortuneWheelPink6");
    }
  }
}
...
class FortuneWheelPink6 extends CommonWheel {
  // constructor
  constructor() {
    super("FortuneWheelPink6"); // scene key FortuneWheel
  }

  setWheelConfig() {
    wheelConfig = wheelConfig_pink_6;
